...
function CompoundingSection({ data, set }) {
  ...
  const upd = (k, v) => set('compounding', s => ({ ...s, [k]: v }));
  ...
  return (
    <>
      <div className="subsec">
        ...
        <ChipGrid value={Array.isArray(c.categories) ? c.categories : []} onChange={v => upd('categories', v)} options={[
      ...
      <div className="subsec">
        ...
        <ChipGrid value={Array.isArray(c.formulations) ? c.formulations : []} onChange={v => upd('formulations', v)} options={[
          'Sublingual troches',
          'Topical creams',
          'Topical gels',
      ...
      <div className="subsec">
        ...
        <Field label="Preferred topical bases">
          <ChipGrid value={Array.isArray(c.bases) ? c.bases : []} onChange={v => upd('bases', v)} options={['VersaBase', 'Lipoderm', 'PCCA Pluronic', 'Aquaphor', 'Eucerin', 'Pentravan']} />
        </Field>
        <div style={{ marginTop: 16 }}>
          <Field label="Flavor preferences (oral/peds)">
            <ChipGrid value={Array.isArray(c.flavorPrefs) ? c.flavorPrefs : []} onChange={v => upd('flavorPrefs', v)} options={['Bubblegum (peds)', 'Cherry', 'Grape', 'Strawberry', 'Bitter mask (adult)', 'Mint', 'Unflavored']} />
          </Field>
        </div>
        <div style={{ marginTop: 16 }}>
...
function PatientSection({ data, set }) {
  ...
  const upd = (k, v) => set('patient', s => ({ ...s, [k]: v }));
  ...
  return (
    <>
      ...
      <div className="subsec">
        ...
        <ChipGrid value={Array.isArray(p.populations) ? p.populations : []} onChange={v => upd('populations', v)} options={[
...
function ShipForm({ value, onSave, onCancel }) {
  ...
  const upd = (k, x) => setV(curr => ({ ...curr, [k]: x }));
...
function BillingSection({ data, set }) {
  ...
  const upd = (k, v) => set('billing', s => ({ ...s, [k]: v }));
          ...
          <div className="subsec">
            ...
            <div className="form">
              ...
              <div className="row-2">
                ...
                <Field label="Account number" required hint="Will be masked after save">
                  <Input mono value={b.accountLast4 ? `••••${b.accountLast4}` : ''} onChange={v => upd('accountLast4', v.slice(-4))} placeholder="0000000000" />
                </Field>
              </div>
            </div>
          </div>
      ...
      <div className="subsec">
        ...
        {open && (
          <div style={{ marginTop: 12, border: '1px solid var(--brand)', borderRadius: 'var(--r-3)', padding: 18, background: 'var(--brand-soft)' }}>
            <div className="form">
              <div className="row-2">
                <Field label="Full name" required><Input value={draft.name} onChange={v => setDraft(d => ({ ...d, name: v }))} placeholder="Daniel Reyes" /></Field>
                <Field label="Role"><Input value={draft.role} onChange={v => setDraft(d => ({ ...d, role: v }))} placeholder="Office Manager" /></Field>
              </div>
              <Field label="Email" required hint="Used for portal invitation"><Input value={draft.email} onChange={v => setDraft(d => ({ ...d, email: v }))} placeholder="name@practice.com" /></Field>
              <Field label="Access level" required>
                <RadioPills value={draft.access} onChange={v => setDraft(d => ({ ...d, access: v }))} options={['View only', 'Order entry', 'Prescribe', 'Full']} />
              </Field>
            </div>
...
function AttestSection({ data, set }) {
  ...
  const upd = (k, v) => set('attestations', s => ({ ...s, [k]: v }));
  ...
  const start = e => {
    ...
    setPaths(ps => [...ps, [{ x: e.clientX - r.left, y: e.clientY - r.top }]]);
  };
  const move = e => {
    if (!drawing) return;
    const r = padRef.current.getBoundingClientRect();
    setPaths(ps => {
      const last = [...ps[ps.length - 1], { x: e.clientX - r.left, y: e.clientY - r.top }];
      return [...ps.slice(0, -1), last];
    });
  };
  const end = () => {
    setDrawing(false);
    setPaths(ps => {
      if (ps.length > 0 && ps.some(p => p.length > 4)) {
        upd('signed', true);
